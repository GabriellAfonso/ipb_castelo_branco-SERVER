...
- **One standing exception**: moving a model between apps. `makemigrations` expresses it as
  `DeleteModel` + `CreateModel`, which drops the table and destroys its rows, so
  `SeparateDatabaseAndState` must be hand-written. Same for `RenameField`, which becomes a
  silent remove-plus-add when generated non-interactively. Both require, as compensating
  controls: the reason at the top of the file, verification against a restored production
  dump rather than an empty database, and a rollback that has actually been run. Precedent:
  `core/migrations/0001_initial.py` and `0002_rename_and_extend.py` (feature 007).
...
at `specs/007-unify-service-catalogue/plan.md`
<!-- SPECKIT END -->
